...
function sir(y,x_t,Δ,obs_fun,σ_o,s,Np,N_thr=10)
    K = size(y)[2]
	...
	w = SharedArray{Float64}(Np)
	w .= 1.0/Np
	x = SharedArray{Float64}(d,Np)
	t = @distributed for i=1:Np
		x[:,i] = rand(d)
		for k = 1:N_runup
			x[:,i] = next(x[:,i],s) 
		end
    ...
    for k = 1:N_y
        if k % Δ == 0
			filter_mean = sum(x,dims=2)/Np
			rmse_trj[j] = norm(filter_mean - x_t[:,k])/sqrt(d)

			t = @distributed for i = 1: Np
                logwi = log(w[i]) + p_y_g_x(y[:,j] .- obs_fun(x[:,i],0), σ_o)
                w[i] = exp(logwi)
...
Perform `K` data assimilation steps using an SIR algorithm with `Np` particles. Other inputs:
	...
	2. `s`: parameter array for the Baker's map (shd make more general)
...
```
"""
function assimilate(K, Np, σ_o, s, 
				   Δ, Nth, obsfun)
	Ny = K*Δ
    ...
    rmse_trj = sir(y, x_true, Δ, obsfun, σ_o, s, Np, Nth)
